...
from nb_init.nb_naming import get_unique_name
        ...
        if isinstance(data, dict):
            ...
            for item_data in data:
                if not 'name' in item_data:
                    id_field=get_unique_name(entity_name, item_data)
                    if id_field is None:
                        print(f"Warning missing name in {entity_name}")
                        continue
                self._create_item(entity_name, item_data[id_field], item_data)
                        
    def _create_item(self, entity_name: str, item_name: str, item_data: Dict[str, Any]):
